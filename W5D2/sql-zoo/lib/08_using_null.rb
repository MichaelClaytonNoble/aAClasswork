...
def all_teachers_join
  # Use a type of JOIN that will list all teachers and their 
  # department,
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT COUNT(*) as num_teachers, COUNT(teachers.mobile) as num_mobiles
  FROM teachers
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  SELECT depts.name, COUNT(teachers.name) as num_teachers
  FROM depts
  LEFT JOIN teachers ON teachers.dept_id = depts.id
  GROUP BY depts.name
...
def teachers_and_divisions
  ...
  SELECT teachers.name,
    CASE 
      WHEN teachers.dept_id = 1 OR teachers.dept_id = 2
        THEN 'Sci'
      ELSE 'Art'
    END
  FROM teachers
